...
module = module.exports = function(parent){
  ...
  Token.prototype = {
    ...
    decode: function decodeToken(token){
      ...
      let tokenString   = fernet.decode64toHex(this.token);
      let versionOffset = fernet.hexBits(8);
      let timeOffset    = versionOffset + fernet.hexBits(64);
      let ivOffset      = timeOffset + fernet.hexBits(128);
      let hmacOffset    = tokenString.length - fernet.hexBits(256);
      let timeInt       = fernet.parseHex(tokenString.slice(versionOffset, timeOffset));

      console.log(tokenString.length);
      console.log(fernet.hexBits(1024));
      ...
      let decodedHmac = fernet.createHmac(this.secret.signingKey, fernet.timeBytes(this.time), this.iv, this.cipherText);
      let decodedHmacHex = decodedHmac.toString(fernet.Hex);
