...
module = module.exports = function(parent){
  ...
  Token.prototype = {
    ...
    decode: function decodeToken(token){
      ...
      var versionOffset = fernet.hexBits(8); // 2
      var timeOffset    = versionOffset + fernet.hexBits(64); // 18
      var ivOffset      = timeOffset + fernet.hexBits(128); // 50
      var hmacOffset    = tokenString.length - fernet.hexBits(256); // 136
