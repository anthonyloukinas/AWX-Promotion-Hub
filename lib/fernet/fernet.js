...
let urlsafe = function urlsafe(string) {
  return string.replace(/\+/g, '-').replace(/\//g, '_') //.replace(/=+$/, '')
};

// parse a Hex string to an Int
let parseHex = function parseHex(hexString) {
  return parseInt('0x' + hexString);
};

// turn bits into number of chars in a hex string
let hexBits = function hexBits(bits) {
  return bits / 8 * 2;
};

// convert base64 string to hex string
let decode64toHex = function decode64(string) {
  let s = URLBase64.decode(string.replace(/=+$/, ''));
  return (new Buffer(s)).toString('hex');
};

// convert array to hex string
let ArrayToHex = function ArrayToHex(array) {
  let hex = '';
  for (let _byte in array) {
    hex += Number(_byte).toString(16).lpad('0',2);
  }
  return hex;
};

let randomHex = function(size){
  return crypto.randomBytes(128/8).toString('hex')
};

let setIV = function setIV(iv_array){
  if (iv_array) {
...
let timeBytes = function timeBytes(time) {
  if(time){
    time = (time / 1000)
  }else{
...
let fernet = function fernet(opts){
  this.Hex = Hex;
  this.Base64 = Base64;
  ...
  this.encryptMessage = function(message, encryptionKey, iv){
    let encrypted = AES.encrypt(message, encryptionKey, {iv: iv});
  ...
  this.createToken = function(signingKey, time, iv, cipherText) {
    let hmac = this.createHmac(signingKey, time, iv, cipherText);
