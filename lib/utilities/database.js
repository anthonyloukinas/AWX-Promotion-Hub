...
exports.updateTowerInstance = (id, name, protocol, host, port, strict_ssl, username, password, callback) => {
  query("UPDATE tower_instances SET name = ?, protocol = ?, host = ?, port = ?, strict_ssl = ?, username = ?, password = ? WHERE id = ?",
      [name, protocol, host, port, strict_ssl, username, password, id], function(error, results) {
      if (error) callback(error, null)
      else callback(null, results);
  });
};