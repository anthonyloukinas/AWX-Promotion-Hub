...
exports.getConfiguredTowers = (callback) => {
  console.log("making the db call now");
  query("SELECT * FROM tower_instances",[], (error, results) => {
    if (error) {
      ...
      console.log("the database call seemed to work?");
...
exports.updateTowerInstance = (id, name, protocol, host, port, strict_ssl, username, password, secret_key, callback) => {
  query("UPDATE tower_instances SET name = ?, protocol = ?, host = ?, port = ?, strict_ssl = ?, username = ?, password = ?, secret_key = ? WHERE id = ?",
      [name, protocol, host, port, strict_ssl, username, password, secret_key, id], function(error, results) {
      if (error) callback(error, null)
      else callback(null, results);
  });
};